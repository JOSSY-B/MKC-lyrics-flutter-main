...
import 'package:mkcchoirlyrics/api/languages_song_api.dart';
...
class LanguagesSongList extends StatefulWidget {
  final String title;
  final String album_id;

  LanguagesSongList({Key? key, required this.title, required this.album_id})
      : super(key: key);

  @override
  _LanguagesSongListState createState() => _LanguagesSongListState();
}

class _LanguagesSongListState extends State<LanguagesSongList> {
  ...
  @override
  void initState() {
    getLanguageSongs().then((value) {
      setState(() {
        _songs.addAll(value);
        _songsDisplay = _songs;
        _songsDisplay =
            _songs.where((val) => val.album_id == widget.album_id).toList();
      });
    });
    super.initState();
                    ...
                    return ListTile(
                      title: Text(song.name,
                          style: const TextStyle(
                              fontSize: 20, fontFamily: 'Abyssinica')),
                      // subtitle: Text(
                      //   "በ${song.year} ዓም",
                      //   style: const TextStyle(
                      //       fontSize: 20,
